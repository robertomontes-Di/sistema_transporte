...
// 7) routes_active: rutas con activa = 1
  // accion 1	Retorno a punto de inicio


  $sql_retorno_punto = "SELECT count(distinct idruta) FROM transport.reporte where idaccion=1;";
  $routes_retorno_punto = (int)$pdo->query($sql_retorno_punto)->fetchColumn();
  //accion17	Salida del Estadio Mágico González

  $sql_salida_evento = "SELECT count(distinct idruta) FROM transport.reporte where idaccion=17";
  $routes_salida_evento = (int)$pdo->query($sql_salida_evento)->fetchColumn();
  
  $sql_en_estadio = "SELECT count(distinct idruta) FROM transport.reporte where idaccion=16";
  $routes_en_estadio = (int)$pdo->query($sql_en_estadio)->fetchColumn();
    // --------------------------------------------------------
...
if ($action === 'map') {
  try {
    ...
    // Obtener suma de personas reportadas por ruta
    $sqlPersonasPorRuta = "
      SELECT idruta, SUM(total_personas) AS total_personas_reportadas
      FROM reporte
      GROUP BY idruta
    ";
    $personasPorRuta = [];
    foreach ($pdo->query($sqlPersonasPorRuta)->fetchAll(PDO::FETCH_ASSOC) as $row) {
    ...
    foreach ($rows as $row) {
      ...
      if (!isset($routes[$idruta])) {
        ...
        $routes[$idruta] = [
          ...
          'total_personas_reportadas' => $personasPorRuta[$idruta] ?? 0,
        ];
      }
...
if ($action === 'recent_reports') {
  try {
    $sql = "
            ...
            LEFT JOIN bus b  ON b.idbus  = rt.idbus
...
<script>
  ...
  // Rutas en transito / Rutas Estimado (en un solo tile)
      ...
      const statusOption = {
        ...
        series: [{
          ...
          data: [{
              value: stats.sin_problema || 0,
              name: 'Sin problema'
            },
            {
              value: stats.inconveniente || 0,
              name: 'Inconveniente'
            },
            {
              value: stats.falla || 0,
  ...
  async function renderMap() {
      ...
      `;
          ...
          // Marcador del bus (ubicación actual compartida)
          if (rt.bus_lat && rt.bus_lng) {
            const busPosMarker = new google.maps.Marker({
              position: {
                lat: rt.bus_lat,
                lng: rt.bus_lng
              },
              ...
              title: `Bus de Ruta ${rt.ruta_nombre} - ${rt.bus_fecha || 'sin fecha'}`
            });

            const busInfo = `
          <div>
            <strong>Total:</strong> ${rt.total_personas_reportadas || 0} personas<br/>
            <strong>Ruta:</strong> ${rt.ruta_nombre}<br/>
            <strong>Última actualización:</strong> ${rt.bus_fecha || 'N/A'}
          </div>
        `;
            ...
            busPosMarker.addListener('click', () => {
              busInfoWindow.open({
                anchor: busPosMarker,
                map: mapInstance
              });
  ...
  async function initDashboard() {
    ...
    // Refresco adicional del mapa cada 10 minutos
    setInterval(async () => {
      await renderMap();
